...
def get_semantic_answer(df, question, explicit_prompt="", model="DaVinci-text", engine='babbage', limit_response=True, tokens_response=100, temperature=0.0):
    ...
    if explicit_prompt == "":
        ...
        if len(res) == 0:
            ...
            prompt = f"{.join(res['text'][0:5]}{restart_sequence}Please reply to the question using only the information present in this text or reply 'Not in the text': {question}"
        else:
